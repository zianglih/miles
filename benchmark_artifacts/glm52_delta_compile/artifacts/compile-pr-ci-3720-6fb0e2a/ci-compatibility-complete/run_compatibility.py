...
SOURCE = ROOT / 'miles'
OUTPUT = ROOT / 'compatibility-results'
...
def run_phase(name, arguments, limit):
    cache = ROOT / (name + '-compatibility-fresh-inductor-cache')
    ...
    for key in ('OMP_NUM_THREADS', 'MKL_NUM_THREADS', 'OPENBLAS_NUM_THREADS', 'TORCHINDUCTOR_COMPILE_THREADS', 'PYTEST_ADDOPTS', 'RAY_ADDRESS', 'LD_LIBRARY_PATH', 'LD_PRELOAD'):
        env.pop(key, None)
    env.update({
        'CUDA_VISIBLE_DEVICES': '', 'PYTHONFAULTHANDLER': '1',
...
if not run_phase('focused', ['tests/fast/utils/test_delta_preparation.py', '-v', '-x'], 600):
    raise SystemExit(1)
