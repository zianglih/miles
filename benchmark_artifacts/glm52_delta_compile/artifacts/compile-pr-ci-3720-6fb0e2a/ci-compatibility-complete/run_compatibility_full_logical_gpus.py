...
SOURCE = ROOT / 'miles'
OUTPUT = ROOT / 'compatibility-full-logical-gpus-results'
...
def run_phase(name, arguments, limit):
    cache = ROOT / (name + '-compatibility-full-logical-gpus-fresh-inductor-cache')
    ...
    for key in ('OMP_NUM_THREADS', 'MKL_NUM_THREADS', 'OPENBLAS_NUM_THREADS', 'TORCHINDUCTOR_COMPILE_THREADS', 'PYTEST_ADDOPTS', 'RAY_ADDRESS', 'LD_LIBRARY_PATH', 'LD_PRELOAD'):
        env.pop(key, None)
    env.update({
        'CUDA_VISIBLE_DEVICES': '99,98,97,96,95,94,93,92', 'PYTHONFAULTHANDLER': '1',
