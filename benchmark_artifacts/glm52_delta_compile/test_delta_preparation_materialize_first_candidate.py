...
def test_compiled_bucket_owns_outputs_and_preserves_per_name_wire(compiled_stage, changed):
    ...
    assert prepared.copied_unchanged_bytes == (128 if changed else 65729)
    assert prepared.copied_padding_bytes == 63
...
def test_compiled_bucket_materializes_unchanged_update_and_preserves_wire_omission(compiled_stage):
    ...
    assert second.snapshot is not first.snapshot and second.xor is not None
    torch.testing.assert_close(second.snapshot, first.snapshot)
    assert not second.xor.any() and not second.changed_bytes
    assert second.copied_unchanged_bytes == 65729
...
def test_reassigning_a_staging_lease_resets_only_padding():
    ...
    assert prepared.snapshot is not old and prepared.changed_counts == (0, 0)
    torch.testing.assert_close(prepared.snapshot, old)
...
def test_word_lanes_tails_aligned_offsets_and_no_change_ownership(value):
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        ...
        assert unchanged.snapshot is not old and not unchanged.snapshot.any()
        assert unchanged.xor is not None and not unchanged.xor.any()
        assert unchanged.copied_unchanged_bytes == 22
        assert unchanged.copied_padding_bytes == 6
