#!/usr/bin/env python3
"""Run the W4A16 recipe with a selected delta CPU backend and compile observations.
...
Example: python launch_weight_sync_compile.py --mode disk-delta --reward-mode
synthetic-balanced --implementation-label candidate --num-rollout 7 --run-name
compiled-synthetic-delta-01 --delta-cpu-backend torch-compile
--compile-cache-dir /hai-workspace/glm52-delta/compiled-integration-cache
...
The v2 CPU/wall helpers remain unchanged. This launcher adds the selected
backend, cache directory, and compiler-counter probe to the recorded manifest.
"""
...
def main():
    ...
    parser.add_argument("--delta-cpu-backend", choices=("numpy", "torch-compile"), required=True)
    ...
    options.compile_cache_dir = options.compile_cache_dir.resolve()
    cache_state_before = {
        "exists": options.compile_cache_dir.exists(),
        "empty": not any(options.compile_cache_dir.iterdir()) if options.compile_cache_dir.exists() else True,
    }
    for repository in (options.repo, options.sglang_repo):
        if subprocess.check_output(
            ["git", "-C", str(repository), "status", "--porcelain", "--untracked-files=normal"], text=True
        ).strip():
            parser.error(f"Validation requires a clean committed source tree: {repository}")
    ...
    def execute_with_probe(**kwargs):
        ...
        train_args += [
            ...
            "weight_sync_probe_compile.install",
            "--update-weight-delta-cpu-backend",
            options.delta_cpu_backend,
        ]
        ...
        kwargs["extra_env_vars"].update(
            {
                ...
                "RAY_DEDUP_LOGS": "0",
                "TORCHINDUCTOR_CACHE_DIR": str(options.compile_cache_dir.resolve()),
        ...
        manifest = {
            ...
            "delta_cpu_backend": options.delta_cpu_backend,
            "compile_observer_version": 1,
            "compile_cache_dir": str(options.compile_cache_dir.resolve()),
            "compile_cache_state_before": cache_state_before,
            ...
            "script_sha256": {
                ...
                for name in (
                    ...
                    "launch_weight_sync_compile.py",
                    "weight_sync_probe_compile.py",
                )
            },
